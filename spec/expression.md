...
- [表达式](#表达式)
  ...
  - [命名空间表达式](#命名空间表达式)
    ...
    - [访问其他模块里的标识符](#访问其他模块里的标识符)
      - [使用 `use` 表达式简化标识符全称](#使用-use-表达式简化标识符全称)
    - [访问同一个命名空间里的标识符](#访问同一个命名空间里的标识符)
...
局部变量定义表达式（`let` 表达式）用于在当前上下文（Environment/Scope）之内创建一个变量（局部变量）并赋值，或者说，将一个值绑定到一个`标识符`（identifier）。
